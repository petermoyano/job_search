...
def test_manual_analysis_routes_are_retired() -> None:
    paths = app.openapi()["paths"]
    assert not any(path == "/jobs" or path.startswith("/jobs/") for path in paths)
    assert "/analyses" not in paths
    assert "/config/scoring" not in paths
    assert "/radar/runs" in paths


def test_candidate_profile_and_cv_extraction_remain_available() -> None:
    with TestClient(app) as client:
        response = client.post("/profiles", json={"name": "Retained profile"})
        assert response.status_code == 201
        response = client.post(
            "/cvs",
            json={
                "candidate_profile_id": response.json()["id"],
                "raw_text": "Senior AI Engineer with Python, PostgreSQL and RAG experience.",
                "use_llm": False,
            },
        )
        assert response.status_code == 201
        assert "Python" in response.json()["extracted_profile"]["technical_skills"]
